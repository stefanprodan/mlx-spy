...
export function mountChat(): ChatPage {
  ...
  function statsRow(m: Message, last: boolean): HTMLElement {
    ...
    // the reply's line covers the whole send: first token of the first
    // round, tokens generated by every round, time from the first round
    const rounds = sendRows(m).filter((x) => x.role === "assistant");
    const first = rounds[0] ?? m;
    const generated = rounds.reduce(
      (sum, x) => sum + (x.stats?.generated ?? 0),
      0,
    );
    const st = m.stats;
    if (first.ttftMs !== null) s.append(span("TTFT", secs(first.ttftMs)));
    if (st) {
      ...
      s.append(p, span("generated", n(Math.max(generated, st.generated))));
    }
    if (m.finishedAt !== null) {
      s.append(span("", secs(m.finishedAt - first.createdAt)));
    }
    if (rounds.length > 1) s.append(span("", `${rounds.length} rounds`));
  ...
  // ---------- work groups ----------
  // the rounds before a send's reply (reasoning, tool calls, results) fold
  // into one block, so the transcript reads question, work, answer
  ...
  // created on the first such round of the send
  function fold(row: HTMLElement) {
    if (row.parentElement?.classList.contains("wb")) return;
  ...
  function workLabel(g: HTMLDetailsElement) {
    ...
    if (tools.length > 0) {
      text += ` · ${tools.length} tool call${tools.length === 1 ? "" : "s"}`;
    }
    if (failed > 0) text += `, ${failed} failed`;
    label.textContent = text;
  }

  // the work is over: the group folds shut and shows its total
  ...
  function onChat(ev: ChatWsEvent) {
    ...
    switch (ev.kind) {
      ...
      case "done": {
        ...
        // that row as its reply, outside the work group
        ...
        if (reply && wb?.classList.contains("wb")) {
          ...
          g.after(reply);
          if (!wb.querySelector(".msg")) g.remove();
        }
        settleAllWork(false);
        renderComposer();
        break;
